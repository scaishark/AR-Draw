    ...
    void DrawOnMouse()
    {
        ...
        if(Input.GetMouseButton(0))
        {
            ...
            if(Lines.Keys.Count == 0)
            {
                ARLine line = new ARLine(lineSettings);
                Lines.Add(0, line);
                line.AddNewLineRenderer(transform, mousePosition);
                //GameObject go = Instantiate(cubePrefab, mousePosition, transform.rotation);
                //line.AddNewCube(transform, mousePosition, go);
